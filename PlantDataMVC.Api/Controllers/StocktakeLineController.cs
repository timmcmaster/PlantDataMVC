...
public class StocktakeLineController : ControllerBase
{
    private readonly IStocktakeLineService _service;
    private readonly IUnitOfWorkAsync _unitOfWorkAsync;
    private readonly IMapper _mapper;
    private readonly ILogger<StocktakeLineController> _logger;

    public StocktakeLineController(IUnitOfWorkAsync unitOfWorkAsync, IStocktakeLineService service, IMapper mapper, ILogger<StocktakeLineController> logger)
    {
        _service = service;
    ...
    [HttpGet(Name = "StocktakeLineList")]
        ...
        [FromQuery] string? suburb = null)
    {
        try
        {
            ...
            if (dsParams.Fields != null)
            {
                ...
                childDataModelsToInclude = DataShaping.GetIncludedObjectNames<StocktakeLineDataModel>(lstOfFields);
            }
            ...
            var dataModels = _mapper
                       .ProjectTo<StocktakeLineDataModel>(context, childDataModelsToInclude.ToArray())
    ...
    public IActionResult GetById(int id, [FromQuery] DataShapingParameters dsParams)
    {
        try
        {
            ...
            var itemDataModel = _mapper.Map<StocktakeLineEntityModel, StocktakeLineDataModel>(item);
    ...
    // POST: api/StocktakeLine
    [HttpPost]
    //[Authorize(Policy = AuthorizationPolicies.RequireWriteUserRole)]
    public IActionResult Post([FromBody] CreateUpdateStocktakeLineDataModel dataModelIn)
    {
        ...
        try
        {
            ...
            var entity = _mapper.Map<CreateUpdateStocktakeLineDataModel, StocktakeLineEntityModel>(dataModelIn);
            _service.Add(entity);
            ...
            if (changes > 0)
            {
                var dataModelOut = _mapper.Map<StocktakeLineEntityModel, StocktakeLineDataModel>(entity);
    ...
    // PUT: api/StocktakeLine/5
    // TODO: Make underlying operation FULL update only (i.e. all stored fields, or default values if not supplied)
    [HttpPut("{id}")]
    //[Authorize(Policy = AuthorizationPolicies.RequireWriteUserRole)]
    public IActionResult Put(int id, [FromBody] CreateUpdateStocktakeLineDataModel dataModelIn)
    {
        try
        {
    ...
    // PATCH: api/StocktakeLine/5
    // Partial update
    [HttpPatch("{id}")]
    //[Authorize(Policy = AuthorizationPolicies.RequireWriteUserRole)]
    public IActionResult Patch(int id, [FromBody] JsonPatchDocument<CreateUpdateStocktakeLineDataModel> itemPatchDoc)
    {
        try
        {
            ...
            // Map to dataModel
            var dataModelFound = _mapper.Map<StocktakeLineEntityModel, CreateUpdateStocktakeLineDataModel>(entityFound);

            // Apply changes to dataModel
            itemPatchDoc.ApplyTo(dataModelFound);

            var updatedEntity = _mapper.Map<CreateUpdateStocktakeLineDataModel, StocktakeLineEntityModel>(dataModelFound);
            updatedEntity.Id = id;
            ...
            if (changes > 0)
            {
                var dataModelOut = _mapper.Map<StocktakeLineEntityModel, StocktakeLineDataModel>(updatedEntity);
