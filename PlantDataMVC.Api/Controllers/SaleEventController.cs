...
namespace PlantDataMVC.Api.Controllers
{
    ...
    {
        private readonly ISaleEventService _service;
        private readonly IUnitOfWorkAsync _unitOfWorkAsync;
        private readonly IMapper _mapper;
        private readonly ILogger<SaleEventController> _logger;

        public SaleEventController(IUnitOfWorkAsync unitOfWorkAsync, ISaleEventService service, IMapper mapper, ILogger<SaleEventController> logger)
        {
            _service = service;
            ...
            [FromQuery] string? name = null)
        {
            try
            {
                ...
                if (dsParams.Fields != null)
                {
                    ...
                    childDataModelsToInclude = DataShaping.GetIncludedObjectNames<SaleEventDataModel>(lstOfFields);
                }
                ...
                var dataModels = _mapper
                           .ProjectTo<SaleEventDataModel>(context, childDataModelsToInclude.ToArray())
        ...
        public IActionResult GetById(int id, [FromQuery] DataShapingParameters dsParams)
        {
            try
            {
                ...
                var itemDataModel = _mapper.Map<SaleEventEntityModel, SaleEventDataModel>(item);
        ...
        public IActionResult Post([FromBody] CreateUpdateSaleEventDataModel dataModelIn)
        {
            ...
            try
            {
                ...
                var entity = _mapper.Map<CreateUpdateSaleEventDataModel, SaleEventEntityModel>(dataModelIn);
                _service.Add(entity);
                ...
                if (changes > 0)
                {
                    var dataModelOut = _mapper.Map<SaleEventEntityModel, SaleEventDataModel>(entity);
        ...
        [HttpPut("{id}")]
        //[Authorize(Policy = AuthorizationPolicies.RequireWriteUserRole)]
        public IActionResult Put(int id, [FromBody] CreateUpdateSaleEventDataModel dataModelIn)
        {
            try
            {
        ...
        // PATCH: api/SaleEvent/5
        // Partial update
        [HttpPatch("{id}")]
        //[Authorize(Policy = AuthorizationPolicies.RequireWriteUserRole)]
        public IActionResult Patch(int id, [FromBody] JsonPatchDocument<CreateUpdateSaleEventDataModel> itemPatchDoc)
        {
            try
            {
                ...
                // Map to dataModel
                var dataModelFound = _mapper.Map<SaleEventEntityModel, CreateUpdateSaleEventDataModel>(entityFound);

                // Apply changes to dataModel
                itemPatchDoc.ApplyTo(dataModelFound);

                var updatedEntity = _mapper.Map<CreateUpdateSaleEventDataModel, SaleEventEntityModel>(dataModelFound);
                updatedEntity.Id = id;
                ...
                if (changes > 0)
                {
                    var dataModelOut = _mapper.Map<SaleEventEntityModel, SaleEventDataModel>(updatedEntity);
