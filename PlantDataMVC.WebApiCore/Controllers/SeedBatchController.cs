    ...
    public class SeedBatchController : ControllerBase
    {
            ...
            int? speciesId = null)
        {
            try
            {
                ...
                var dtos = _mapper
                           .ProjectTo<SeedBatchDto>(context, childDtosToInclude.ToArray())
        ...
        public IActionResult GetById(int id, [FromQuery] DataShapingParameters dsParams)
        {
            try
            {
                ...
                var itemDto = _mapper.Map<SeedBatchEntityModel, SeedBatchDto>(item);
        ...
        public IActionResult Post([FromBody] CreateUpdateSeedBatchDto dtoIn)
        {
            ...
            try
            {
                ...
                var entity = _mapper.Map<CreateUpdateSeedBatchDto, SeedBatchEntityModel>(dtoIn);
                _service.Add(entity);
                ...
                if (changes > 0)
                {
                    var dtoOut = _mapper.Map<SeedBatchEntityModel, SeedBatchDto>(entity);
        ...
        public IActionResult Patch(int id, [FromBody] JsonPatchDocument<CreateUpdateSeedBatchDto> itemPatchDoc)
        {
            try
            {
                ...
                // Map to dto
                var dtoFound = _mapper.Map<SeedBatchEntityModel, CreateUpdateSeedBatchDto>(entityFound);

                // Apply changes to dto
                itemPatchDoc.ApplyTo(dtoFound);

                var updatedEntity = _mapper.Map<CreateUpdateSeedBatchDto, SeedBatchEntityModel>(dtoFound);
                updatedEntity.Id = id;
                ...
                if (changes > 0)
                {
                    var dtoOut = _mapper.Map<SeedBatchEntityModel, SeedBatchDto>(updatedEntity);
