    ...
    public class SaleEventController : ControllerBase
    {
            ...
            [FromQuery] string? name = null)
        {
            try
            {
                ...
                var dtos = _mapper
                           .ProjectTo<SaleEventDto>(context, childDtosToInclude.ToArray())
        ...
        public IActionResult GetById(int id, [FromQuery] DataShapingParameters dsParams)
        {
            try
            {
                ...
                var itemDto = _mapper.Map<SaleEventEntityModel, SaleEventDto>(item);
        ...
        public IActionResult Post([FromBody] CreateUpdateSaleEventDto dtoIn)
        {
            ...
            try
            {
                ...
                var entity = _mapper.Map<CreateUpdateSaleEventDto, SaleEventEntityModel>(dtoIn);
                _service.Add(entity);
                ...
                if (changes > 0)
                {
                    var dtoOut = _mapper.Map<SaleEventEntityModel, SaleEventDto>(entity);
        ...
        [HttpPut("{id}")]
        //[Authorize(Policy = AuthorizationPolicies.RequireWriteUserRole)]
        public IActionResult Put(int id, [FromBody] CreateUpdateSaleEventDto dtoIn)
        {
            try
            {
        ...
        [HttpPatch("{id}")]
        //[Authorize(Policy = AuthorizationPolicies.RequireWriteUserRole)]
        public IActionResult Patch(int id, [FromBody] JsonPatchDocument<CreateUpdateSaleEventDto> itemPatchDoc)
        {
            try
            {
                ...
                // Map to dto
                var dtoFound = _mapper.Map<SaleEventEntityModel, CreateUpdateSaleEventDto>(entityFound);

                // Apply changes to dto
                itemPatchDoc.ApplyTo(dtoFound);

                var updatedEntity = _mapper.Map<CreateUpdateSaleEventDto, SaleEventEntityModel>(dtoFound);
                updatedEntity.Id = id;
                ...
                if (changes > 0)
                {
                    var dtoOut = _mapper.Map<SaleEventEntityModel, SaleEventDto>(updatedEntity);
